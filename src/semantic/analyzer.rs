...
impl SemanticAnalyzer {
    ...
    fn analyze_expression(&mut self, expr: &Expression) -> StriaResult<Type> {
        match expr {
            Expression::Literal(literal) => {
                Ok(self.get_literal_type(&literal.value))
            }
            ...
            Expression::Call(call) => {
                ...
                if let Expression::Identifier(func_name, _) = call.callee.as_ref() {
                    let func_decl = self.functions.get(func_name)
                        .ok_or_else(|| StriaError::semantic(format!(
                    ...
                    for (arg, param) in call.arguments.iter().zip(func_decl.parameters.iter()) {
                        ...
                        if let Some(ref param_type_annotation) = param.type_annotation {
                            let param_type = self.type_annotation_to_type(param_type_annotation)?;
            ...
            Expression::MemberAccess(member_access) => {
                ...
                match &object_type {
                    Type::Struct(struct_name) => {
                        ...
                        let property = struct_decl.properties.iter()
            ...
            Expression::StructInstantiation(struct_instantiation) => {
                let struct_decl = self.structs.get(&struct_instantiation.name)
                    .ok_or_else(|| StriaError::semantic(format!(
                        "Undefined struct '{}'", struct_instantiation.name
                    )))?.clone();
                ...
                if struct_instantiation.arguments.len() != struct_decl.properties.len() {
                    return Err(StriaError::semantic(format!(
                        ...
                        struct_instantiation.name, struct_decl.properties.len(), struct_instantiation.arguments.len()
                    )));
                }
                
                // Check argument types
                for (arg, property) in struct_instantiation.arguments.iter().zip(struct_decl.properties.iter()) {
                    let arg_type = self.analyze_expression(arg)?;
                    if let Some(ref type_annotation) = property.type_annotation {
    ...
    fn analyze_binary_operation(&self, op: &BinaryOperator, left: &Type, right: &Type) -> StriaResult<Type> {
        match op {
    ...
    fn is_compatible_type(&self, expected: &Type, actual: &Type) -> bool {
        expected == actual || 
        (self.is_numeric_type(expected) && self.is_numeric_type(actual))
    }
    ...
    fn statement_has_return(&self, stmt: &Statement) -> bool {
        match stmt {
            ...
            Statement::Expression(Expression::If(if_expr)) => {
                let then_has_return = if let Expression::Block(then_block) = if_expr.then_branch.as_ref() {
